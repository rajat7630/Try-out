  ...
  <header>
	...
	<div class="container">
       {#if currentRoute.name ==="/showproblems"}
				...
				<div>
					<input type="text" id="myInput" on:keyup={myFunction} placeholder="Search for Problems.." title="Type in a name" style ="width:90%">
				...
				<tbody class="bg-grey-light flex flex-col items-center justify-between overflow-y-scroll w-full" style="height: 50vh;">
					...
					<tr class="flex w-full mb-4">
						...
						{#each result.data.allProblems as prob}
							<tr class="flex w-full mb-4" on:click={() => {window.location =`/problem/${prob.id}`}} id= "pp">
        ...
        {:else if currentRoute.name === "/showtests"}
					...
					<div>
						<input type="text" id="myInput" on:keyup={myFunction} placeholder="Search for Tests.." title="Type in a name" style ="width:90%">
					...
					<tbody class="bg-grey-light flex flex-col items-center justify-between overflow-y-scroll w-full" style="height: 50vh;">
						...
						<tr class="flex w-full mb-4">
							...
							{#each result.data.allTests as tes}
								<tr class="flex w-full mb-4" id ="pp" on:click={ () => {window.location =`/test/${tes.id}`}}>
        ...
        {:else if currentRoute.name === "/showresults"}
					...
					<h1 class="mb-8">Scoreboard: </h1>
						<input type="text" id="myInput" on:keyup={myFunction} placeholder="Search for users.." title="Type in a name" style ="width:100%">
					<table id ="myTable" class="text-left w-full">
					<thead class="bg-black flex text-white w-full">
						<tr class="flex w-full mb-4">
